...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $up = $pdo->prepare(
            "UPDATE peminjaman SET user_id=?, buku_id=?, tanggal_pinjam=?, tanggal_kembali=?, status=? WHERE id=?"
        );
        $up->execute([
            $user_id,
            $buku_id,
        ...
        header("Location: index.php?success=Peminjaman berhasil diperbarui!");
        exit;
    }
}
?>
    ...
    <div class="card" style="max-width:520px;">
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <input type="date" name="tanggal_kembali" value="<?= htmlspecialchars((string)$tanggal_kembali) ?>">
            </div>
            ...
            <button type="submit" class="btn btn-warning" style="width:100%; padding:12px; font-size:15px;">Update Peminjaman</button>
        </form>
    </div>
</div>
</body>
